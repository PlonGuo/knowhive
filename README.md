...
- **Self-built cross-session memory, proven with an A/B eval** — a distillation-based long-term memory (semantic / procedural / episodic) that remembers durable facts across separate conversations. Measured: on the 6 user-specific questions that survived a leak self-check, memory lifts answer recall from **0% → 100%**; across all 8 candidates before that check the OFF arm scores 0.25, because two anchors turned out to be derivable from the corpus and were struck rather than counted ([learnings/evals/Memory-Eval.md](learnings/evals/Memory-Eval.md)).
...
| Does my memory system actually help? | Answer recall **0% → 100%** on the 6 questions that passed a leak self-check (the raw 8-question OFF arm is 0.25 — two anchors were derivable from the corpus and were struck, not counted) | [Memory-Eval](learnings/evals/Memory-Eval.md) |
